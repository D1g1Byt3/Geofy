package com.digibyte.geofy.command;
...
public class ReturnHomeCommand {
    ...
    private int returnHome(CommandSourceStack source) throws CommandSyntaxException {
        ...
        boolean hasHomepos = player.getPersistentData().getIntArray(GeofyMod.MOD_ID + "homepos").length != 0;

        if(hasHomepos) {
            int[] playerPos = player.getPersistentData().getIntArray(GeofyMod.MOD_ID + "homepos");
